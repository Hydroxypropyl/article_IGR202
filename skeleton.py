...
def build_graph(skeleton, junction_coordinates : list):
    ...
    """
    G=nx.Graph()  
    junction_neighbours, edges_list = get_neighbours_and_edges(skeleton, junction_coordinates)
    for i in range(len(junction_coordinates)):
        G.add_node(i)
        edges = [(i, j) for j in junction_neighbours[i]]
        G.add_edges_from(edges)
    return G


def local_extrema(edge : list, start:tuple, end:tuple) :
    max_dist = 0
    extrema = start
    ux = end[0] - start[0]
    uy = end[1] - start[1]
...
def get_control_points_bezier(edge : list, P0 : tuple, P3 : tuple) -> list:
    """
    ...
    x3 = P3[0]
    y3 = P3[1]
    extrema = local_extrema(edge, P0, P3)
    print(P0, P3, extrema)
    xm = extrema[0]
    ym = extrema[1]
...
def fitting_error(edge : list, control_points : list) -> float:
    """
...
def fit_bezier_curve(edge : list, start :tuple, end : tuple, junction_coordinates :list) :
    """
    ...
    if len(edge)> 3 :     
        control_points = get_control_points_bezier(edge, start, end)
        err = fitting_error(edge, control_points)
        #print(err)
        if err > 20 :
            min_err = err
            split_index = 0
            for i in range(2,len(edge)-2) :
                err1 = fitting_error(edge[:i+1], get_control_points_bezier(edge[:i+1], edge[0], edge[i]))
                err2 = fitting_error(edge[i:], get_control_points_bezier(edge[i:], edge[i], edge[-1]))
                print("err1 {}, err {}".format(err1 + err2, err))
                if err1 + err2 < min_err :
                    split_index = i
                    min_err = err1 + err2
            split_point = edge[split_index]
            print(split_point)
            junction_coordinates.append(edge[split_index])
            fit_bezier_curve(edge[:split_index+1], start, split_point, junction_coordinates)
            fit_bezier_curve(edge[split_index:], split_point, end, junction_coordinates)
...
def topological_graph(skeleton) -> tuple[list, list] : 
    ...
    for i in range(len(nodes_coordinates)) :
        start = nodes_coordinates[i]
        print("fitting edge {} out of {}".format(i, n))
        edges = edges_list[i]
        for edge in edges : 
            print("edge size : ", len(edge))
            new_nodes_coordinates = fit_bezier_curve(edge, start, edge[-1], new_nodes_coordinates)
    ...
    return nodes_coordinates, nodes_neighbours, edges_list
...
def test() :
    ...
    nodes_coordinates, nodes_neighbours, edges_list = topological_graph(skeleton)

    y = [nodes_coordinates[i][0] for i in range(len(nodes_coordinates))]
    x = [nodes_coordinates[i][1] for i in range(len(nodes_coordinates))]
    plt.plot(x, y, 'ro')
    plt.show()
