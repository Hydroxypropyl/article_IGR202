...
def get_skeleton(img) -> list:
    """
    This function takes as input the trapped ball segmented image. 
    It then computes the skeleton by intersecting iteratively all the zones.
    Parameters : 
    -----------------
...
def is_junction(skeleton : list, pixel : tuple) :
    """
    Checks whether a certain pixel is a junction node. 
    A pixel is considered junction if it has at least three neighbour in 4-connexity.
...
def get_junctions(skeleton : list) -> list:
    """
    ...
    for i in range(height):
        for j in range(width):
            if skeleton[i,j] > 0 : 
                ...
                if bool == True :
                    junctions_coordinates.append((i,j))
    return junctions_coordinates


def neighbours_4_connexity(img : list, p_i : int, p_j : int) -> list:
    """
    ...
    Returns : 
    -----------
    A list of tuples containing the coordinates (i,j) of the neighbours in `img`

    """
    neighbours = []
    candidates = [(p_i-1, p_j), (p_i, p_j-1), (p_i, p_j+1), (p_i+1, p_j)]
...
def get_edges(skeleton : list, junction_coordinates : list) -> tuple[list, list]:
    """
    Computes the edges determined by white pixels in `skeleton`.
    ...
    Returns : 
    -------------------
    `edges_list` : a list of same length as `junction_coordinates`, 
    where edges_list[i] is the list of edges(all the pixels) for junction_coordinates[i]. 
    Edges contain the coordinates of the pixels and contain their start and end pixels.
    """
    print("Computing edges list...")
    edges_list =[]
    junction_neighbours=[]
    width = len(skeleton[0])
    i = 1
        ...
        for n in range(nb_potential_edges) : 
            ...
            while (edge[-1]//width, edge[-1]%width) not in junction_coordinates and not_edge == False :
                ...
                if len(neighbours) < 2 : 
                    ...
                    y = pixel//width
                    x = pixel%width
                    print(skeleton[y-1: y+2, x-1:x+2])
                    not_edge = True
                else : 
...
def flatten_coordinates(width : int, junction_coordinates : list)-> list:
    """
    Parameters : 
    ---------------
...
def local_extrema(edge : list, start : int, end : int, width : int) -> tuple [float, float] :
    """
    Computes the point in the edge that maximizes the distance to its projection onto the line (start, end).
...
def get_control_points_bezier(edge : list, P0 : int, P3 : int, width : int, deg = 3) -> list:
    ...
    If deg == 3 then a total of 4 control points is outputted.
    """
    ...
    # end point should always be added
    coordinates.append((y3, x3))
    return coordinates


def bezier_curve(tp : tuple, control_points : list) -> tuple[float, float]:
    """
    Computes the Bezier curve determined by `control_points` at normalized position `tp` along the edge.
    The degree of the Bezier curve is deducted from the number of control points : \n
    if len(`control_points`) = n, the degree of the Bezier curve is n-1. 
    
    Parameters : 
    ------------------
    `tp` : tuple of 2 elements containing normalized coordinates along initial curve.
    `control_points` : list of control points for the curve. Must contain between 2 and 4 points included, 
...
def compute_bezier_curve(edges_list : list, junctions_indices : list , width : int) -> list[list]:
    """
    Computes iteratively the Bezier curve for all the edges in `edges_list`. All Bezier curves are of degree 3.

    Parameters : 
    ----------------
    `edges_list` : 
    `junctions_indices` : indices of the junctions in the flattened image.
    `width` : width of the image, used to convert the coordinates into image coordinates.

    Returns : 
    ----------------
    `curve` : a list of length 2 where `curve[0]` are the x coordinates of all the Bezier curves
     and `curve[1]` are the corresponding y coordinates.
    """
    curve = [[],[]]
    for P0 in range(len(junctions_indices)) :
        ...
        for edge in edges :
            ...
            y = []
            if len(edge)>2:
                control_points = get_control_points_bezier(edge, edge[0], edge[-1], width) #of degree 3
                norm = len(edge)
                for i in range(norm) :
                    tp =i/norm
...
def fit_bezier_curve(edge : list, start :int, end : int, junction_coordinates :list, width : int, threshold = 100) -> list :
    """
    Takes `edge` and recursively fits Bezier curve on different segments by splitting in new segments, 
    in order to get a fitting error below `threshold`.
    Parameters : 
    ---------------------
    ...
    Returns : 
    --------------------
    `junction_coordinates` : the updated junction coordinates such that the fitting error of a curve
...
def topological_graph(skeleton : list , name) -> tuple[list, list] : 
    """
    
    Parameters : 
    -----------------
    `skeleton` : image in black and white, [0, 255]
    `name` : name of the file. Used to save the differents plots.
    ...
    #junction_neighbours, edges_list = get_edges(skeleton, nodes_coordinates)
    junctions_indices = flatten_coordinates(width, nodes_coordinates)
    #edges_list = get_edges(G, junctions_indices)
    edges_list = get_edges(skeleton, nodes_coordinates)
    ...
    #nodes_neighbours, edges_list = get_edges(skeleton, new_nodes_coordinates)
    junctions_indices = flatten_coordinates(width, new_nodes_coordinates)
    ...
    final_edges = get_edges(skeleton, junctions_coordinates)
...
def test(img_name) :
    ...
    inverted_skeleton = 255*np.ones(skeleton.shape) - skeleton
    plt.imshow(skeleton, cmap='gray')
    plt.title("skeleton")
    plt.show()

    cv.imwrite("results/"+name+"_skeleton.png", skeleton)
    cv.imwrite("results/"+name+"_inverted_skeleton.png", inverted_skeleton)
...
#### Check number of final edges
    ...
    if True :
        ...
        plt.plot(curves[1], curves[0], color = 'black', marker = 'o', linestyle = 'none', ms=0.5)
        ax = plt.gca()
        ax.set_xlim(0,len(skeleton[0]))
        ax.set_ylim(len(skeleton),0)
        ax.set_aspect('equal', adjustable='box')
